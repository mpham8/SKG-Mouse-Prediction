...
def filter_data(x, y, core_thresh, opp_thresh):
	...
	# 	for c in classes:
	# 		print('new')
	# 		print(num_counts[str(c)].loc[feat]) #ERROR HERE
	# 		print(core_thresh)
	# 		print(num_counts[str(c)].loc[feat] >= core_thresh)
...
def prepare_data(path, data):
	...
	try:
		g = pickle.load(open(path + "/PopPhy-tree-core.pkl", 'rb')) #edited
		...
		pickle.dump(g, open(path + "/PopPhy-tree-core.pkl", 'wb')) #edited
